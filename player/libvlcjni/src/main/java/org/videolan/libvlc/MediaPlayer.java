...
//        public static final int ChapterChanged      = 0x11d;
    ...
    /**
     * libvlcjni-3.x track descriptor (id + name). Looked up by the VLC 3 JNI_OnLoad; the VLC 4
     * track API returns full {@link Media.Track} objects instead.
     */
    ...
    /* libvlcjni-3.x: stop() resets the audio output, so it has to be applied again before the next play() */
    private boolean mAudioReset = false;
    /* VLC 3 needs an explicit default aout (android_audiotrack); VLC 4 chooses one itself */
    private String mAudioOutput = LibVLC.isVlc3() ? "android_audiotrack" : null;
    ...
    public void updateVideoSurfaces() {
        if (mVideoHelper != null) 
            mVideoHelper.updateVideoSurfaces();
    }

    /**
     * Update the video dimensions in VLC core
     */
    public void updateVideoDimensions() {
        if (mVideoHelper != null) 
            mVideoHelper.updateVideoDimensions();
    }
    ...
    public void play() {
        synchronized (this) {
            if (!mPlaying) {
                /* libvlcjni-3.x HACK: stop() reset the audio output, so set it again before first play. */
                if (mAudioReset) {
    ...
    public void setScale(float scale) {
        if (LibVLC.isVlc3()) {
            /* libvlcjni-3.x: the Java VideoHelper lays the surface out itself; scale goes straight to the core */
    ...
    public int getDisplayFit() {
        /* VLC 3 has no display-fit; the Java VideoHelper always fits inside the surface */
        if (LibVLC.isVlc3())
            return FitMode.Smaller;
    ...
    public Media.Track getSelectedTrack(int type) {
        if (LibVLC.isVlc3()) {
            final int selectedId = legacyGetSelectedTrackId(type);
            if (selectedId == -1)
                return null;
            final Media.Track[] tracks = legacyMediaTracks(type);
            if (tracks == null)
                return null;
            final String wanted = Media.legacyTrackId(type, selectedId);
            for (Media.Track track : tracks)
                if (wanted.equals(track.id))
                    return track;
            return null;
        }
    ...
    public Media.Track[] getSelectedTracks(int type) {
        if (LibVLC.isVlc3()) {
            final Media.Track selected = getSelectedTrack(type);
            return selected != null ? new Media.Track[] { selected } : null;
        }
    ...
    public Media.Track getTrackFromID(String id) {
        if (LibVLC.isVlc3()) {
            final Media.Track[] tracks = legacyMediaTracks(Media.Track.Type.Unknown);
            if (tracks != null)
                for (Media.Track track : tracks)
                    if (id != null && id.equals(track.id))
                        return track;
            return null;
        }
    ...
    public boolean selectTrack(String id) {
        if (LibVLC.isVlc3())
            return legacySetTrack(legacyTrackType(id), Media.legacyTrackIntId(id));
        return nativeSelectTrack(id);
    }

    /* ---- libvlcjni-3.x track plumbing: the VLC 3 core only knows integer ids per ES category ---- */

    /** Tracks of one type from the current media (VLC 3 has no player-level track list). All types when type is Unknown. */
    private Media.Track[] legacyMediaTracks(int type) {
        final IMedia media = mMedia;
        if (media == null || media.isReleased())
            return null;
        return type == Media.Track.Type.Unknown ? media.getTracks() : media.getTracks(type);
    }

    private static int legacyTrackType(String id) {
        if (id == null) return Media.Track.Type.Unknown;
        if (id.startsWith("audio/")) return Media.Track.Type.Audio;
        if (id.startsWith("video/")) return Media.Track.Type.Video;
        if (id.startsWith("spu/")) return Media.Track.Type.Text;
        return Media.Track.Type.Unknown;
    }

    private int legacyGetSelectedTrackId(int type) {
        switch (type) {
            case Media.Track.Type.Audio: return nativeGetAudioTrack();
            case Media.Track.Type.Video: return nativeGetVideoTrack();
            case Media.Track.Type.Text: return nativeGetSpuTrack();
            default: return -1;
        }
    }

    private boolean legacySetTrack(int type, int id) {
        switch (type) {
            case Media.Track.Type.Audio: return nativeSetAudioTrack(id);
            case Media.Track.Type.Video:
                /* Don't activate a video track if surfaces are not ready */
                if (id == -1 || (mWindow.areViewsAttached() && !mWindow.areSurfacesWaiting()))
                    return nativeSetVideoTrack(id);
                return false;
            case Media.Track.Type.Text: return nativeSetSpuTrack(id);
            default: return false;
        }
    }

    /** libvlcjni-3.x API: id/name descriptors of the video tracks known to the player. */
    public TrackDescription[] getVideoTracks() {
        return LibVLC.isVlc3() ? nativeGetVideoTracks() : legacyDescriptions(Media.Track.Type.Video);
    }

    /** libvlcjni-3.x API: id/name descriptors of the audio tracks known to the player. */
    public TrackDescription[] getAudioTracks() {
        return LibVLC.isVlc3() ? nativeGetAudioTracks() : legacyDescriptions(Media.Track.Type.Audio);
    }

    /** libvlcjni-3.x API: id/name descriptors of the subtitle tracks known to the player. */
    public TrackDescription[] getSpuTracks() {
        return LibVLC.isVlc3() ? nativeGetSpuTracks() : legacyDescriptions(Media.Track.Type.Text);
    }

    private TrackDescription[] legacyDescriptions(int type) {
        final Media.Track[] tracks = getTracks(type);
        if (tracks == null)
            return null;
        final TrackDescription[] descriptions = new TrackDescription[tracks.length];
        for (int i = 0; i < tracks.length; ++i)
            descriptions[i] = new TrackDescription(Media.legacyTrackIntId(tracks[i].id), tracks[i].name);
        return descriptions;
    }
    ...
    {
        if (LibVLC.isVlc3()) {
            /* VLC 3 can only select one track per type: take the first id of the list */
            if (ids == null)
                return;
            final int comma = ids.indexOf(',');
            legacySetTrack(type, Media.legacyTrackIntId(comma >= 0 ? ids.substring(0, comma) : ids));
            return;
        }
    ...
    public void setVideoTrackEnabled(boolean enabled) {
        if (LibVLC.isVlc3()) {
            /* libvlcjni-3.x behaviour: pick the first real video track by its integer id */
            if (!enabled) {
                legacySetTrack(Media.Track.Type.Video, -1);
            } else if (!isReleased() && hasMedia() && nativeGetVideoTrack() == -1) {
                final TrackDescription[] tracks = nativeGetVideoTracks();
                if (tracks != null) {
                    for (TrackDescription track : tracks) {
                        if (track.id != -1) {
                            legacySetTrack(Media.Track.Type.Video, track.id);
                            break;
                        }
                    }
                }
    ...
    public boolean record(String directory, boolean enable) {
        /* VLC 3 starts on a directory and stops on null (see nativeRecord) */
        if (LibVLC.isVlc3())
            return nativeRecord(enable ? directory : null, enable);
    ...
    /* VLC 4 only: string-id based track API */
    ...
    /* libvlcjni-3.x only: integer-id based track API */
    ...
    /*
     * VLC 4 signature. libvlcjni-3.x exports the same symbol taking only the directory; the extra
     * boolean is ignored by the C ABI, so recording on VLC 3 starts on a non-null directory and
     * stops on null exactly like upstream 3.x did.
     */
